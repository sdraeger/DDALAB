    ...
    if (data?.getEdfData) {
      ...
      if (convertedData) {
        ...
        try {
          ...
          logger.warn("Failed to cache plot data:", cacheError);
          toast({
            title: "Cache Warning",
            description:
              "Plot data loaded but could not be cached. Performance may be slower on subsequent loads.",
            variant: "default",
          });
        }
        ...
        if (annotations && filePath) {
          try {
            ...
            logger.warn("Failed to cache annotations:", cacheError);
          }
        }
      } else {
        setManualErrorMessage(
