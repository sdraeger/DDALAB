...
class NetworkMotifData:
    ...
    def from_json(cls, payload: dict) -> "NetworkMotifData":
        return cls(
            num_nodes=int(_json_key(payload, "numNodes", "num_nodes", 0)),
            node_labels=[
                str(value)
                for value in (
                    payload.get("nodeLabels") or payload.get("node_labels") or []
                )
            ...
            delay_values=[
                float(value)
                for value in (
                    payload.get("delayValues") or payload.get("delay_values") or []
                )
                if value is not None
            ],
        )
...
class DdaVariantResult:
        ...
        return cls(
            ...
            network_motifs=(
                NetworkMotifData.from_json(
                    payload.get("networkMotifs") or payload.get("network_motifs")
                )
                if isinstance(
                    payload.get("networkMotifs") or payload.get("network_motifs"),
                    dict,
    ...
    def row_mean_absolute_value(self, index: int) -> float:
        ...
        if not row:
            return 0.0
        finite = [abs(float(value)) for value in row if math.isfinite(float(value))]
        if not finite:
            return 0.0
...
class DdaResult:
    ...
    def from_json(cls, payload: dict) -> "DdaResult":
        return cls(
            ...
            engine_label=str(_json_key(payload, "engineLabel", "engine_label", "")),
            ...
            is_fallback=bool(_json_key(payload, "isFallback", "is_fallback", False)),
            reproduction=(
...
class DdaReproductionConfig:
    ...
    def from_json(cls, payload: dict) -> "DdaReproductionConfig":
        ...
        return cls(
            ...
            compute_device=str(
                payload.get("computeDevice") or payload.get("compute_device") or "cpu"
            ).lower(),
            variant_ids=_str_list(
                payload.get("variantIds") or payload.get("variant_ids")
            ),
            ...
            variant_pair_indices=_pair_index_map(
                payload.get("variantPairIndices") or payload.get("variant_pair_indices")
            ),
            variant_pair_names=_pair_name_map(
                payload.get("variantPairNames") or payload.get("variant_pair_names")
            ),
            ...
            model_dimension=int(
                payload.get("modelDimension") or payload.get("model_dimension") or 0
            ),
            polynomial_order=int(
                payload.get("polynomialOrder") or payload.get("polynomial_order") or 0
            ),
