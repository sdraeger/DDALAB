...
_DDA_FALLBACK_SQL = """
(
    COALESCE(is_fallback, 0) = 1
    OR payload_json LIKE '%"is_fallback":true%'
    OR payload_json LIKE '%"isFallback":true%'
)
"""
...
_TIMESTAMPED_TABLE_ID_COLUMNS = {
    "notifications": "notification_id",
    "workflow_actions": "action_id",
    "workflow_sessions": "session_id",
}


def _default_database_path() -> Path:
    path = Path.home() / ".ddalab" / "state.sqlite3"
    legacy_path = Path.home() / ".ddalab-qt" / "state.sqlite3"
    if path.exists() or not legacy_path.exists():
        return path

    path.parent.mkdir(parents=True, exist_ok=True)
    temporary_path = path.with_suffix(".migrating")
...
class StateDatabase:
    def __init__(self, db_path: Optional[Path] = None) -> None:
        ...
        self._sql = _SqliteStore(self.db_path)
        self._init_schema()
    ...
    def _ensure_column(
        ...
        table_sql = self._sql.identifier(
            table_name,
            allowed=_STATE_TABLES,
            kind="table name",
        )
        column_sql = self._sql.identifier(
            column_name,
            allowed=_MIGRATION_COLUMNS.get(table_name, set()),
            kind="column name",
        )
        definition_sql = self._sql.literal_sql(
            definition,
            allowed=_MIGRATION_DEFINITIONS,
            kind="column definition",
        )
        existing = {
            str(row["name"])
            for row in self._sql.fetchall(f"PRAGMA table_info({table_sql})")
        }
        if column_name in existing:
            return
        self._sql.execute(
            f"ALTER TABLE {table_sql} ADD COLUMN {column_sql} {definition_sql}"
        )
        self._sql.commit()
    ...
    def _has_persisted_state(self) -> bool:
        ...
        for query in checks:
            row = self._sql.execute(query).fetchone()
            if row is not None:
                return True
        return False
    ...
    def load_session_payload(self) -> dict:
        payload: dict = {}
        for row in self._sql.execute("SELECT key, value_json FROM session_state"):
    ...
    def replace_annotations_for_file(
        ...
        with self._sql.transaction():
            ...
            if annotation_list:
                placeholders = self._sql.placeholders(len(annotation_list))
                self._sql.execute(
                    f"""
    ...
    def save_dda_result(self, result: DdaResult) -> None:
        ...
        if result.is_fallback:
            with self._sql.transaction():
                self._sql.execute(
                    "DELETE FROM dda_results WHERE result_id = ?",
                    (result.id,),
                )
            return
    ...
    def purge_fallback_dda_results(self) -> int:
        before_changes = self._sql.total_changes
        with self._sql.transaction():
            self._sql.execute(
                f"""
                DELETE FROM dda_results
                WHERE {_DDA_FALLBACK_SQL}
                """
            )
        return self._sql.total_changes - before_changes
    ...
    def replace_notifications(self, entries: Iterable[NotificationEntry]) -> None:
        ...
        self._replace_timestamped_payload_rows(
            ...
            "notification_id",
            (
                (entry.id, entry.created_at_iso, self._dumps(asdict(entry)))
    ...
    def replace_workflow_actions(self, actions: Iterable[WorkflowActionEntry]) -> None:
        ...
        self._replace_timestamped_payload_rows(
            ...
            "action_id",
            (
                (action.id, action.created_at_iso, self._dumps(asdict(action)))
    ...
    def replace_workflow_sessions(
        ...
        self._replace_timestamped_payload_rows(
            ...
            "session_id",
            (
                (session.id, session.created_at_iso, self._dumps(asdict(session)))
    ...
    def _replace_timestamped_payload_rows(
        ...
        expected_id_column = _TIMESTAMPED_TABLE_ID_COLUMNS.get(table_name)
        if expected_id_column != id_column:
            raise ValueError(
                f"Unsupported timestamped payload target: {table_name!r}.{id_column!r}"
            )
        table_sql = self._sql.identifier(
            table_name,
            allowed=_TIMESTAMPED_TABLE_ID_COLUMNS,
            kind="table name",
        )
        id_column_sql = self._sql.identifier(
            id_column,
            allowed={expected_id_column},
            kind="column name",
        )
        row_list = list(rows)
        with self._sql.transaction():
            for row_id, created_at_iso, payload_json in row_list:
                self._sql.execute(
                    f"""
                    INSERT INTO {table_sql}({id_column_sql}, created_at_iso, payload_json)
                    VALUES (?, ?, ?)
                    ON CONFLICT({id_column_sql}) DO UPDATE SET
                        created_at_iso=excluded.created_at_iso,
                        payload_json=excluded.payload_json
                    """,
            ...
            if row_list:
                placeholders = self._sql.placeholders(len(row_list))
                self._sql.execute(
                    f"DELETE FROM {table_sql} WHERE {id_column_sql} NOT IN ({placeholders})",
                    [row_id for row_id, _, _ in row_list],
                )
            else:
                self._sql.execute(f"DELETE FROM {table_sql}")
    ...
    def _deserialize_dda_result(self, payload: object) -> DdaResult:
        ...
        variants = [
            DdaVariantResult(
                ...
                row_labels=[
                    str(value)
                    for value in (item.get("row_labels") or item.get("rowLabels") or [])
                ],
                matrix=[
                ...
                min_value=float(item.get("min_value") or item.get("minValue") or 0.0),
                max_value=float(item.get("max_value") or item.get("maxValue") or 0.0),
                column_count=int(
                    item.get("column_count") or item.get("columnCount") or 0
                ),
                row_mean_absolute=[
                    float(value)
                    for value in (
                        item.get("row_mean_absolute")
                        or item.get("rowMeanAbsolute")
                        or []
                    )
                ],
                row_peak_absolute=[
                    float(value)
                    for value in (
                        item.get("row_peak_absolute")
                        or item.get("rowPeakAbsolute")
                        or []
                    )
                ],
                network_motifs=(
                    NetworkMotifData.from_json(
                        item.get("network_motifs") or item.get("networkMotifs")
                    )
                    if isinstance(
                        item.get("network_motifs") or item.get("networkMotifs"),
                        dict,
        ...
        return DdaResult(
            ...
            reproduction=(
                DdaReproductionConfig.from_json(data["reproduction"])
    ...
    def _deserialize_dda_result_summary(self, row: sqlite3.Row) -> DdaResultSummary:
        ...
        engine_label = str(row["engine_label"] or "")
        is_fallback = (
            bool(int(row["is_fallback"])) if row["is_fallback"] is not None else False
        )
