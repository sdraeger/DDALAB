...
class DdaSidecarClient:
    ...
    def cuda_devices(self) -> list[dict[str, object]]:
        payload = self.request("devices")
    ...
    def _request_locked(
        ...
        *,
        ...
        if process is None or process.poll() is not None:
            raise RuntimeError(
                self._dead_process_message("DDA sidecar is not running.")
            )
        stdin = process.stdin
        ...
        while True:
            ...
            if response.get("ok") is not True:
                message = str(
                    response.get("error") or f"DDA sidecar '{method}' failed."
                )
