...
def _subset_reproduction(
    ...
    variant_ids = (
        [variant_id] if variant_id else list(repro.variant_ids or available_variant_ids)
    )
    if not variant_ids:
    ...
    if selected_channel_names and len(selected_channel_names) != len(
    ...
    return DdaReproductionConfig(
        ...
        compute_device=repro.compute_device,
...
def _build_reproduction_cli_args(
    ...
    model_dimension = int(reproduction.model_dimension or _DEFAULT_DDA_MODEL_DIMENSION)
    ...
    args: list[str] = [
        ...
        *[str(int(term)) for term in model_terms],
        ...
        str(nr_tau),
        "--device",
        reproduction.compute_device or "cpu",
        "--start",
    ...
    if reproduction.selected_channel_indices:
        args.extend(
            [
                "--channels",
                *[str(int(index)) for index in reproduction.selected_channel_indices],
            ]
        )
