...
class _FileBrowserItemDelegate(QStyledItemDelegate):
    def paint(self, painter: QPainter, option, index) -> None:  # type: ignore[override]
        ...
        if index.column() == 0:
            self._paint_name_column(
                painter, option.rect, colors, appearance, entry, selected
            )
        elif index.column() == 1:
            self._paint_type_column(
                painter, option.rect, colors, appearance, entry, selected
            )
        else:
    ...
    def _paint_name_column(
        ...
        text_rect = QRectF(
            rect.left() + 28, rect.top(), rect.width() - 36, rect.height()
        )
        font = painter.font()
        ...
        name = metrics.elidedText(
            entry.name, Qt.TextElideMode.ElideMiddle, int(text_rect.width())
        )
        painter.setPen(
            QColor(colors.selection_text) if selected else appearance.name_color
        )
        painter.drawText(
    ...
    def _paint_marker(
        ...
        border = (
            QColor(colors.selection_text) if selected else QColor(appearance.accent)
        )
    ...
    def _paint_type_column(
        ...
        for badge_index, token in enumerate(tokens):
            ...
            text = metrics.elidedText(
                token, Qt.TextElideMode.ElideRight, int(remaining - 20)
            )
            width = min(metrics.horizontalAdvance(text) + 18, int(remaining))
            badge_rect = QRectF(
                x, badge_area.center().y() - (badge_height / 2), width, badge_height
            )
    ...
    def _paint_badge(
        ...
        badge_fill.setAlpha(
            44 if primary and not selected else 24 if not selected else 26
        )
        badge_border = (
            QColor(colors.selection_text) if selected else QColor(appearance.accent)
        )
        badge_border.setAlpha(
            120 if primary and not selected else 68 if not selected else 120
        )
        badge_text = (
            QColor(colors.selection_text) if selected else QColor(appearance.accent)
        )
    ...
    def _paint_size_column(
        ...
        painter.setPen(
            QColor(colors.selection_text) if selected else QColor(colors.text_muted)
        )
        painter.drawText(
    ...
    def _show_context_menu(self, position) -> None:
        ...
        if chosen is copy_full_label:
            QApplication.clipboard().setText(
                _entry_tooltip(entry, _entry_type_label(entry))
            )
...
def _entry_tooltip(entry: BrowserEntry, type_label: str) -> str:
    ...
    if type_label == "BIDS Dataset":
        lines.append(
            "Open datasets from the subject, session, or modality folders inside this root."
        )
    elif "BrainVision Companion" in type_label:
        lines.append(
            "Opening this file resolves through its matching BrainVision .vhdr header."
        )
