...
class MainWindowVisualizeMixin:
    ...
    def _update_dataset_ui(self) -> None:
        ...
        for key, value in {
            ...
            "duration": f"{dataset.duration_seconds:.2f}s",
            "channels": str(len(dataset.channels)),
            "samples": str(dataset.total_sample_count),
        }.items():
            label = getattr(self, "overview_cards", {}).get(key)
    ...
    def _apply_annotations_to_views(self) -> None:
        ...
        quick_waveform_bridge = getattr(self, "quick_waveform_bridge", None)
        if quick_waveform_bridge is not None and hasattr(
            quick_waveform_bridge,
            "set_annotations",
        ):
            quick_waveform_bridge.set_annotations(annotations)
