...
from ..support.main_window_support import _human_bytes
...
class MainWindowIntegrationsMixin:
    ...
    def _backend_supports_nsg_submission(self) -> bool:
        return bool(getattr(self.backend, "supports_nsg_submission", lambda: False)())
    ...
    def _update_nsg_panels(self) -> None:
        ...
        self.nsg_delete_credentials_button.setEnabled(nsg_enabled and creds is not None)
    ...
    def _load_openneuro(self, append: bool = False) -> None:
        ...
        self.status_bar.showMessage(
            "Loading more OpenNeuro datasets…"
            if append
            else "Loading OpenNeuro datasets…",
            3000,
        )
    ...
    def _export_all_result_csv(self) -> None:
        def continue_export(result: DdaResult) -> None:
            ...
            target_path, _ = QFileDialog.getSaveFileName(
                ...
                str(
                    Path.home() / f"{default_result_base_name(result)}-all-variants.csv"
                ),
                "CSV Files (*.csv)",
            )
            if not target_path:
    ...
    def _export_result_script(self, format_name: str) -> None:
        def continue_export(result: DdaResult) -> None:
            ...
            default_path = (
                Path.home() / f"{default_result_base_name(result)}-{suffix}.{extension}"
            )
    ...
    def _import_annotations(self) -> None:
        ...
        def on_success(result: object) -> None:
            ...
            if not isinstance(restored, dict) or not restored:
                ...
                return
            for file_path, imported_annotations in restored.items():
                if not isinstance(file_path, str) or not isinstance(
                    imported_annotations, list
                ):
                    continue
                self.state.annotations_by_file[file_path] = list(imported_annotations)
                self.state_db.replace_annotations_for_file(
                    file_path, imported_annotations
                )
            active_file_path = (
                self.state.active_file_path
                if isinstance(self.state.active_file_path, str)
            ...
            if active_file_path and active_file_path in restored:
                ...
                if (
                    ...
                    and Path(preferred_file_path).exists()
                ):
                            ...
                            **(
                                {
                                    ...
                                    "ddaResult": asdict(result.materialize())
                                    if result
                                    else None,
                                    "icaResult": asdict(ica_result)
                                    if ica_result
                                    else None,
                                }
                                if mode != "recipe_only"
    ...
    def _import_workflow(self) -> None:
        ...
        def on_payload(payload: dict) -> None:
            ...
            self._update_workflow_ui()
            self._notify(
                "import", "info", "Action Log Imported", Path(source_path).name
            )
