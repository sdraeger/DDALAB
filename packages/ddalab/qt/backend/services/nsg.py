...
class NsgClient:
    ...
    def test_connection(self) -> bool:
        response = self._request(
            "GET", f"{self.base_url}/job/{self.username}", timeout=30
        )
        return response.ok or response.status_code == 404

    def list_user_jobs(self) -> List[tuple[str, str]]:
        response = self._request(
            "GET", f"{self.base_url}/job/{self.username}", timeout=60
        )
...
class LocalNsgManager:
    ...
    def list_jobs(self) -> List[NsgJobSnapshot]:
        ...
        local_by_nsg_id = {job.nsg_job_id: job for job in local_jobs if job.nsg_job_id}
    ...
    def refresh_job(self, job_id: str) -> NsgJobSnapshot:
        ...
        if record.status in _TERMINAL_NSG_STATUSES:
            record.completed_at = (
                status_payload["completed_at"] or record.completed_at or _utcnow_iso()
            )
        record.output_files = [
    ...
    def download_results(self, job_id: str) -> List[str]:
        ...
        if not output_files:
            raise RuntimeError(
                "NSG returned no downloadable output files for this job."
            )
