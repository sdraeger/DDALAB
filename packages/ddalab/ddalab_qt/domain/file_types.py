...
def resolve_dataset_path(path: str, is_directory: bool = False) -> str:
    target = Path(path)
    if is_directory:
        return str(target)
    brainvision_header = _brainvision_header_for_eeg(target)
    if brainvision_header is not None:
        return str(brainvision_header)
    return str(target)
...
def classify_path(path: str, is_directory: bool) -> PathTypeInfo:
    ...
    if suffix == ".eeg":
        if _brainvision_header_for_eeg(target) is not None:
            return _with_bids_context(target, "BrainVision Companion", True)
        return _with_bids_context(target, "EEG File", False)
    if suffix == ".vhdr":
...
def _brainvision_header_for_eeg(path: Path) -> Optional[Path]:
    if _path_suffix(path) != ".eeg":
        return None
    try:
    ...
    for sibling in siblings:
        if not sibling.is_file():
            continue
        if sibling.suffix.lower() != ".vhdr":
            continue
        if sibling.stem.lower() != lower_stem:
            continue
        if sibling.name == f"{path.stem}.vhdr":
            return sibling
        preferred = sibling
    return preferred
