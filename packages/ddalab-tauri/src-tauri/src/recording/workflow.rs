...
use super::actions::{WorkflowNode, WorkflowEdge, DependencyType};
...
impl WorkflowGraph {
    ...
    pub fn add_edge(&mut self, edge: WorkflowEdge) -> Result<()> {
        ...
        self.graph.add_edge(*source_idx, *target_idx, edge.dependency_type);
    ...
    pub fn remove_node(&mut self, node_id: &str) -> Result<()> {
        let index = self.node_map.remove(node_id)
    ...
    pub fn get_topological_order(&self) -> Result<Vec<String>> {
        let sorted_indices = toposort(&self.graph, None)
            .map_err(|_| anyhow!("Graph contains cycles"))?;
...
mod tests {
    use super::*;
    use crate::recording::actions::{WorkflowAction, FileType};
    ...
    fn test_add_edge() {
        ...
        let node1 = WorkflowNode::new("node1".to_string(), WorkflowAction::LoadFile {
            path: "/test/file.edf".to_string(),
            file_type: FileType::EDF,
        });
        let node2 = WorkflowNode::new("node2".to_string(), WorkflowAction::RunDDAAnalysis {
    ...
    fn test_topological_sort() {
        ...
        let node3 = WorkflowNode::new("node3".to_string(), WorkflowAction::ExportResults {
