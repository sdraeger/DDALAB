...
export class AnnotationStateModule implements FileStateModule<FileAnnotationState> {
  readonly moduleId = 'annotations'

  async loadState(filePath: string): Promise<FileAnnotationState | null> {
    try {
      const state = await invoke<FileAnnotationState>('get_file_annotation_state', {
        filePath,
      })
      return state
    } catch (error) {
