...
export function PersistenceDebugger() {
  ...
  const testBasicInvoke = async () => {
    try {
      ...
      addLog(
        `Basic invoke success: ${JSON.stringify(result).slice(0, 100)}...`,
      );
  ...
  const testLoad = async () => {
    try {
      ...
      addLog(
        `Load operation success: ${JSON.stringify(result).slice(0, 100)}...`,
      );
      setSavedState(result);
    } catch (error) {
  ...
  useEffect(() => {
    ...
    addLog(
      `window.__TAURI__ exists: ${typeof window !== "undefined" && "__TAURI__" in window}`,
    );
    testBasicInvoke();
  }, []);
