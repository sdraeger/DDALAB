  ...
  function PaperReproductionBrowser() {
    ...
    return (
      <div className="space-y-6">
        ...
        {recipesError && (
          <Alert variant={hasRecipes ? "warning" : "destructive"}>
            <AlertCircle className="h-4 w-4" />
            <AlertTitle>
              {hasRecipes
                ? "Paper recipe refresh failed"
                : "Paper recipes unavailable"}
            </AlertTitle>
        ...
        {!recipesError && !hasRecipes && (
          <div className="flex flex-col items-center justify-center py-12 text-muted-foreground space-y-3">
            <BookOpen className="h-12 w-12 opacity-40" />
            <p className="text-sm font-medium text-foreground">
              No paper recipes are currently published
            </p>
            <p className="max-w-md text-center text-sm">
              DDALAB did not receive any paper reproduction recipes from the
              remote catalog. Try refreshing again later.
            </p>
            <Button
              variant="outline"
              size="sm"
                ...
                <CardContent className="space-y-3">
                  ...
                  {activeRecipeId === recipe.id ? (
                    <div className="rounded-md bg-amber-50 dark:bg-amber-950/30 px-3 py-2 text-sm text-amber-700 dark:text-amber-300">
                      Recipe execution coming soon. The full reproduction
