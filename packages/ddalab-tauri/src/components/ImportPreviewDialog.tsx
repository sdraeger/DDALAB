...
export function ImportPreviewDialog({
  ...
  const handleConfirm = async () => {
    ...
    try {
      await onConfirm(
        previewData.import_file_path,
        previewData.target_file,
  ...
  return (
    ...
    >
      <DialogContent className="max-w-6xl max-h-[90vh] flex flex-col">
        ...
        {previewData.is_multi_file_export &&
          previewData.available_files.length > 0 && (
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1 text-sm">
                  ...
                  <p className="text-blue-700 dark:text-blue-300 mb-2">
                    Showing annotations from all files. Annotations will be
                    imported into the currently selected file.
                  </p>
                  <details className="text-xs">
                    <summary className="cursor-pointer text-blue-600 dark:text-blue-400 hover:underline">
                      View all {previewData.available_files.length} files in
                      export
                    </summary>
                      ...
                      {previewData.available_files.map((file) => (
                        <li key={file.path} className="text-muted-foreground">
                          ...
                          <span className="ml-2 text-xs">
                            ({file.annotation_count} annotation
                            {file.annotation_count !== 1 ? "s" : ""})
                          </span>
                        </li>
                      ))}
                    </ul>
        ...
        {previewData.warnings.length > 0 && (
          <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-lg p-3 mb-4">
            <div className="font-semibold text-yellow-700 dark:text-yellow-500 mb-2">
              Warnings:
            </div>
        ...
        <div className="flex items-center gap-2 border-b pb-3">
          ...
          <Button variant="outline" size="sm" onClick={selectOnlyNew}>
            Only New
          </Button>
        </div>
              ...
              {previewData.annotations.map((ann) => (
                ...
                >
                  ...
                  <td className="p-2 text-sm">
                    {ann.channel ? (
                      <Badge variant="outline">{ann.channel}</Badge>
                    ) : (
                  ...
                  <td className="p-2 text-sm">
                    {ann.closest_existing && (
                      <div className="text-xs">
                        <div>
                          Closest: "{ann.closest_existing.label}" at{" "}
                          {formatTime(ann.closest_existing.position)}
                        </div>
                        <div className="text-muted-foreground">
                          Δ: {ann.closest_existing.time_diff > 0 ? "+" : ""}
                          {ann.closest_existing.time_diff.toFixed(2)}s
                        </div>
                      </div>
                    )}
                  </td>
        ...
        <DialogFooter>
          ...
          >
            {isImporting
              ? "Importing..."
              : `Import ${selectedIds.size} Annotation${selectedIds.size !== 1 ? "s" : ""}`}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
