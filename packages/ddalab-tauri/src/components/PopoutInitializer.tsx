...
export function PopoutInitializer({ children }: PopoutInitializerProps) {
  ...
  useEffect(() => {
    const initialize = async () => {
      ...
      try {
        console.log(
          "[PopoutInitializer] Initializing FileStateSystem for popout window...",
        );
        await initializeFileStateSystem();
        isInitializedRef.current = true;
        console.log(
          "[PopoutInitializer] FileStateSystem initialized successfully",
        );
      } catch (error) {
        console.error(
          "[PopoutInitializer] Failed to initialize FileStateSystem:",
          error,
        );
      }
    };
