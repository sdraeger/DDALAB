  ...
  useEffect(() => {
    ...
    const createHeatmap = () => {
      ...
      channels.forEach((channel) => {
        const channelData = (currentVariant.dda_matrix[channel] || []).map(
          (val) => {
    ...
    const createLinePlot = () => {
      ...
      const plotData: uPlot.AlignedData = [
        ...
        ...channels.map(
          (channel) =>
            new Float64Array(currentVariant.dda_matrix[channel] || []),
        ),
      ];
          ...
          <CardContent className="flex items-center justify-center h-48">
            <div className="text-center text-muted-foreground">
              ...
              <p className="text-sm mt-2">
                This analysis may not have completed successfully
              </p>
            </div>
          </CardContent>
        </Card>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
              {analysis.parameters.variants && (
                <div>
                  ...
                  <p className="font-medium">
                    {analysis.parameters.variants.join(", ")}
                  </p>
                </div>
              )}
              ...
              {analysis.parameters.start_time !== undefined &&
                analysis.parameters.end_time !== undefined && (
                  <div>
                    <p className="text-muted-foreground">Time Range</p>
                    <p className="font-medium">
                      {analysis.parameters.start_time}s -{" "}
                      {analysis.parameters.end_time}s
                    </p>
                  </div>
                )}
            </div>
