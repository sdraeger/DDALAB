...
export function PerformanceMonitor() {
  ...
  const [report, setReport] = useState<ReturnType<typeof profiler.generateReport> | null>(null);
  const [memoryUsage, setMemoryUsage] = useState<ReturnType<typeof profiler.getMemoryUsage> | null>(null);
      ...
      <DialogContent className="max-w-4xl max-h-[80vh]">
        ...
        <ScrollArea className="h-[60vh]">
          <div className="space-y-4">
              ...
              <CardContent>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <div className="text-2xl font-bold">{report?.metrics.length || 0}</div>
                  </div>
                  <div>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-sm flex items-center gap-2">
                    Bottlenecks
                    <Badge variant="destructive">{report.bottlenecks.length}</Badge>
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="space-y-2">
                    {report.bottlenecks.slice(0, 10).map((metric, i) => (
                      ...
                      >
                        ...
                        <Badge variant="outline">{metric.duration?.toFixed(2)}ms</Badge>
                      </div>
                    ))}
                  </div>
                </CardContent>
            ...
            {/* Averages */}
            {report && Object.keys(report.averages).some((k) => report.averages[k] > 0) && (
              <Card>
                <CardHeader>
                  <CardTitle className="text-sm">Average Times by Category</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="space-y-2">
                    {Object.entries(report.averages)
                      ...
                      .map(([category, avg]) => (
                        <div key={category} className="flex justify-between items-center">
                          ...
                          <span className="font-mono text-sm">{avg.toFixed(2)}ms</span>
                        </div>
                      ))}
                  </div>
                </CardContent>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="space-y-1">
                    {report.metrics.slice(-20).reverse().map((metric, i) => (
                      <div
                        key={i}
