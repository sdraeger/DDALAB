...
export function useNSGJobs(options?: { enabled?: boolean; refetchInterval?: number }) {
  return useQuery({
  ...
  return useMutation({
    ...
    onSuccess: (updatedJob) => {
      ...
      queryClient.setQueryData<NSGJob[]>(nsgKeys.jobs(), (oldJobs) => {
        ...
        return oldJobs.map((job) => (job.id === updatedJob.id ? updatedJob : job))
      })
...
export function useExtractNSGTarball() {
  return useMutation({
    mutationFn: async ({ jobId, tarFilePath }: { jobId: string; tarFilePath: string }) => {
      if (!TauriService.isTauri()) {
        throw new Error('NSG operations only available in Tauri')
      }
...
export function useInvalidateNSGCache() {
  ...
  return {
    invalidateCredentials: () => queryClient.invalidateQueries({ queryKey: nsgKeys.credentials() }),
    invalidateJobs: () => queryClient.invalidateQueries({ queryKey: nsgKeys.jobs() }),
    invalidateJob: (jobId: string) => queryClient.invalidateQueries({ queryKey: nsgKeys.job(jobId) }),
    invalidateJobStatus: (jobId: string) =>
      queryClient.invalidateQueries({ queryKey: nsgKeys.jobStatus(jobId) }),
    invalidateAll: () => queryClient.invalidateQueries({ queryKey: nsgKeys.all }),
  }
}
