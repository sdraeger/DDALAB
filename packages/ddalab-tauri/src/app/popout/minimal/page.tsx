...
function PopoutContent() {
  ...
  const [linePlotDimensions, setLinePlotDimensions] = useState({
    width: 0,
    height: 0,
  });
  ...
  const renderDDAHeatmap = useCallback(() => {
    ...
    try {
      ...
      uplotHeatmapRef.current = new uPlot(
        opts,
        plotData,
        ddaHeatmapRef.current,
      );
  ...
  const renderDDALinePlot = useCallback(() => {
    ...
    try {
      ...
      channels.forEach((channel: string) => {
        if (
          dda_matrix[channel] &&
          Array.isArray(dda_matrix[channel]) &&
          dda_matrix[channel].length > 0
        ) {
      ...
      uplotLinePlotRef.current = new uPlot(
        opts,
        plotData,
        ddaLinePlotRef.current,
      );
    ...
    return (
      <div className="p-4 h-full flex flex-col">
        ...
        {result.results?.exponents &&
          Object.keys(result.results.exponents).length > 0 && (
            <div className="mb-4">
                  ...
                  <div className="font-semibold">
                    {(() => {
                      ...
                      return exps.length > 0
                        ? (
                            exps.reduce((a, b) => a + b, 0) / exps.length
                          ).toFixed(3)
                        : "N/A";
                    })()}
                  </div>
      ...
      <div className="h-10 bg-gray-100 border-b flex items-center justify-between px-3 select-none">
        ...
        <div className="flex items-center space-x-1">
          <button
            ...
            title={
              isLocked
                ? "Unlock window (currently not receiving updates)"
                : "Lock window (stop receiving updates)"
            }
          >
            {isLocked ? "🔒" : "🔓"}
          </button>
