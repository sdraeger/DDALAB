      ...
      dockPositions.forEach((pos) => {
        if (
          ...
          if (distance < minDistance && distance <= tolerance) {
            ...
            const hasOverlap = otherWidgets.some(
              (w) => w !== other && CollisionDetector.checkOverlap(testRect, w)
            );
...
class SnapGuide {
  static findAlignmentGuides(
    ...
    otherWidgets.forEach((other) => {
      ...
      if (
        Math.abs(
          draggedWidget.x + draggedWidget.width - other.x - other.width
        ) <= tolerance
      ) {
      ...
      if (
        Math.abs(
          draggedWidget.y + draggedWidget.height - other.y - other.height
        ) <= tolerance
      ) {
...
export function DashboardGrid({
  ...
  const handleMouseMove = useCallback(
    (e: MouseEvent) => {
      ...
      if (enableSnapping) {
        ...
        const otherWidgets = widgets
          .filter((w: any) => w.id !== dragState.widgetId)
          .map((w: any) => ({
            x: w.position.x,
            y: w.position.y,
            width: w.size.width,
      ...
      dispatch(
        moveWidget({ id: dragState.widgetId, position: { x: newX, y: newY } })
      );
    },
    [
      isDragging,
  ...
  return (
    ...
    >
      ...
      {/* Snap guides */}
      {enableSnapping &&
        (snapGuides.x !== undefined || snapGuides.y !== undefined) && (
          <>
      ...
      {/* Loading, empty state, or widgets */}
      {!isInitialized ? (
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="text-center p-8 max-w-md">
            ...
            <p className="text-muted-foreground">
              Fetching your saved layout and widgets...
            </p>
          </div>
        </div>
      ) : widgets.length === 0 ? (
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="text-center p-8 max-w-md">
            ...
            <p className="text-muted-foreground mb-4">
              Add widgets to customize your dashboard and visualize your data.
            </p>
            <div className="bg-muted p-4 rounded-lg text-left">
              <h4 className="font-medium mb-2">How to add widgets:</h4>
              <ol className="list-decimal list-inside space-y-1 text-sm text-muted-foreground">
                <li>Click the "Add Widget" button in the header</li>
                <li>Select a widget type from the dialog</li>
                <li>Configure your widget settings</li>
                <li>Place it anywhere on the dashboard</li>
              </ol>
            </div>
          </div>
        </div>
