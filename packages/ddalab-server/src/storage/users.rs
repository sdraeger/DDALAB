...
pub trait UserStore: Send + Sync {
    ...
    /// Update user's institution assignment
    async fn set_user_institution(&self, id: Uuid, institution_id: Option<Uuid>) -> StorageResult<()>;
...
impl PostgresUserStore {
    ...
    pub async fn initialize(&self) -> StorageResult<()> {
        ...
        // Add institution_id column if it doesn't exist (for existing deployments)
        let _ = sqlx::query("ALTER TABLE users ADD COLUMN IF NOT EXISTS institution_id UUID")
            .execute(&self.pool)
            .await;
...
impl UserStore for PostgresUserStore {
    async fn create_user(&self, user: CreateUser) -> StorageResult<User> {
        ...
        sqlx::query(
            r#"
            INSERT INTO users (id, email, display_name, password_hash, is_admin, is_active, institution_id, created_at)
    ...
    async fn get_user(&self, id: Uuid) -> StorageResult<User> {
        let row = sqlx::query(
            r#"
            SELECT id, email, display_name, password_hash, is_admin, is_active, institution_id, created_at, last_login
            FROM users
            WHERE id = $1
            "#,
        )
    ...
    async fn set_user_institution(&self, id: Uuid, institution_id: Option<Uuid>) -> StorageResult<()> {
        let result = sqlx::query(
            r#"
            UPDATE users SET institution_id = $2 WHERE id = $1
            "#,
        )
        .bind(id)
        .bind(institution_id)
