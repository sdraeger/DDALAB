...
# Path to local DDALAB installation (for setup files)
DDALAB_LOCAL_PATH=${userConfig.projectLocation || "./"}
...
API_PORT_METRICS=${userConfig.apiPortMetrics || "8002"}
...
`;
  ...
  static updateEnvContent(
    ...
    const updates = {
      ...
      API_PORT_METRICS: userConfig.apiPortMetrics || "8002",
      HOSTNAME: "0.0.0.0",
      PORT: userConfig.webPort || "3000",
      PYTHON_API_URL: "http://api:8000",
      API_BASE_URL: `http://localhost:${userConfig.apiPort || "8001"}`,
      NEXTAUTH_SECRET: "dev-nextauth-secret-key-for-development-only",
    };
...
`;
  ...
  static async updateDockerCompose(
    ...
    try {
      ...
      // Read the copied template content
      let composeContent = await fs.readFile(composePath, "utf-8");
  ...
  static async copyDockerComposeTemplate(targetDir: string): Promise<void> {
    ...
    // If docker-compose.yml already exists (copied from source), skip template
    try {
      await fs.access(composePath);
      logger.info(`docker-compose.yml already exists at ${composePath}, skipping template copy`);
      return;
    } catch {
      // File doesn't exist, continue with template copy
    }
    ...
    try {
      ...
      try {
        ...
        // Don't throw - we may have already copied the file from source
        logger.warn(`Could not copy docker-compose template, but continuing with setup`);
      }
    }
  }
  ...
  static async cloneRepository(
    ...
    try {
      logger.info(`Starting repository clone/setup for directory: ${targetDir}`);
      ...
      try {
        ...
        logger.error(`Failed to create target directory: ${mkdirError.message}`);
        throw new Error(`Failed to create directory ${targetDir}: ${mkdirError.message}`);
      }
      ...
      // Get the path to the DDALAB installation files
      let localDDALABPath: string;
      
      if (app.isPackaged) {
        // In production, use bundled resources
        // Resources are extracted to a temporary location or within the app bundle
        localDDALABPath = path.join(process.resourcesPath, 'app', 'dist', 'resources', 'ddalab-files');
        logger.info(`Using bundled DDALAB files from: ${localDDALABPath}`);
        
        // Verify bundled files exist
        try {
          await fs.access(path.join(localDDALABPath, 'docker-compose.yml'));
        } catch (error) {
          // Try alternative paths for different packaging scenarios
          const alternativePaths = [
            path.join(process.resourcesPath, 'app', 'resources', 'ddalab-files'),
            path.join(app.getAppPath(), 'resources', 'ddalab-files'),
            path.join(app.getAppPath(), 'dist', 'resources', 'ddalab-files'),
          ];
          
          let found = false;
          for (const altPath of alternativePaths) {
            try {
              await fs.access(path.join(altPath, 'docker-compose.yml'));
              localDDALABPath = altPath;
              logger.info(`Found bundled files at: ${altPath}`);
              found = true;
              break;
            } catch {
              continue;
          ...
          if (!found) {
            throw new Error('Bundled DDALAB files not found. Please reinstall the application.');
          }
        }
      } else {
        // In development, use environment variable or find the files
        localDDALABPath = process.env.DDALAB_LOCAL_PATH || '';
        
        if (!localDDALABPath) {
          // Try to find the docker-compose.yml in common locations
          const possiblePaths = [
            path.join(__dirname, '../../../../'), // From build output back to project root
            path.join(process.cwd(), '../../../'), // From configmanager package to root
            path.join(app.getAppPath(), '../../../'), // From app path to root
            '/Users/simon/Desktop/DDALAB', // Fallback to known location
          ];
          
          for (const testPath of possiblePaths) {
            try {
              await fs.access(path.join(testPath, 'docker-compose.yml'));
              localDDALABPath = testPath;
              logger.info(`Found DDALAB installation at: ${testPath}`);
              break;
            } catch {
          ...
          if (!localDDALABPath) {
            throw new Error('Could not locate DDALAB installation files. Please ensure DDALAB_LOCAL_PATH environment variable is set.');
          }
        }
      }
      ...
      mainWindow.webContents.send("setup-progress", {
        message: `Copying DDALAB configuration files...`,
      });

      logger.info(`Copying essential files from DDALAB directory: ${localDDALABPath}`);

      // Copy the main docker-compose.yml file
      const sourceComposePath = path.join(localDDALABPath, "docker-compose.yml");
      ...
      for (const file of filesToCopy) {
        const sourceFile = path.join(localDDALABPath, file);
      ...
      for (const dir of directoriesToCopy) {
        const sourceDir = path.join(localDDALABPath, dir);
