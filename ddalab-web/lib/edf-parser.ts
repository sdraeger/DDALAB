  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        // Create simulated data based on the filename
        const simulatedData = createSimulatedEEGData(file.name)
        console.log(`Created simulated EEG data with ${simulatedData.channels.length} channels`)
        resolve(simulatedData)
      } catch (error) {
        console.error("Error parsing EDF file:", error)
        // Fall back to simulated data on error
        const simulatedData = createSimulatedEEGData(file.name)
        console.log(`Falling back to simulated data with ${simulatedData.channels.length} channels`)
        resolve(simulatedData)
      }
    }
...
function generateSimulatedEEGData(channelCount: number, samplesPerChannel: number): number[][] {
  const data: number[][] = []
  ...
  for (let channel = 0; channel < channelCount; channel++) {
    ...
    for (let segment = 0; segment < segments; segment++) {
      ...
      const segmentEnd = Math.min(segmentStart + samplesPerSegment, samplesPerChannel)
      ...
      for (let i = segmentStart; i < segmentEnd; i++) {
        ...
        // Alpha waves (8-13 Hz)
        sample += baseAmp * Math.sin((2 * Math.PI * baseFreq * i) / samplesPerChannel)

        // Add some beta waves (13-30 Hz)
        sample += (baseAmp / 2) * Math.sin((2 * Math.PI * (baseFreq * 2) * i) / samplesPerChannel)

        // Add some theta waves (4-8 Hz)
        sample += (baseAmp / 3) * Math.sin((2 * Math.PI * (baseFreq / 2) * i) / samplesPerChannel)
        ...
        if (Math.random() < 0.0005) {
          sample += Math.random() * baseAmp * 5 * (Math.random() > 0.5 ? 1 : -1)
        }
