...
async function createDirectusUser(userData, adminToken) {
  ...
  try {
    ...
    const directusUserData = {
      ...
      password: userData.password, // Directus will hash this
    ...
    console.error('- User data (partial):', { email: userData.email, first_name: userData.firstName, last_name: userData.lastName });
...
const userAuth = {
   ...
   */
  async validateInviteCode(code, email = null) {
    const query = `
   ...
   */
  async registerWithInviteCode(userData, inviteCode) {
    ...
    try {
      ...
      try {
        ...
        await createDirectusUser({
          email,
          password, // Plain password as Directus will hash it
          firstName,
          lastName
        }, adminToken);
        
        console.log('User successfully created in both PostgreSQL and Directus');
      } catch (directusError) {
        // Log Directus error but don't fail the registration
        console.error('Warning: User created in database but failed to create in Directus:', directusError);
  ...
  async authenticateUser(username, password) {
    ...
    const query = `
      SELECT id, username, password_hash, email, first_name, last_name, is_admin
      FROM users
