    ...
    if (url.startsWith("https://")) {
      try {
        if (existsSync(SSL_CERT_PATH)) {
          ...
          console.warn(
            `SSL certificate not found at ${SSL_CERT_PATH}, proceeding without it`
          );
          options.rejectUnauthorized = false;
        }
      } catch (error) {
        console.warn(
          `Error loading certificate: ${error.message}, proceeding without it`
        );
        options.rejectUnauthorized = false;
      }
    }
      ...
      res.on("end", () => {
        if (res.statusCode >= 200 && res.statusCode < 300) {
          ...
          try {
            ...
            reject(
              new Error(
                `Sync failed: ${errorResponse.detail || errorResponse.message || "Unknown error"}`
              )
            );
          } catch (error) {
