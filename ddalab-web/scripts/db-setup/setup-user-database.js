...
if (!config.directusEmail || !config.directusPassword) {
  console.error(
    "Error: DIRECTUS_EMAIL and DIRECTUS_PASSWORD environment variables are required"
  );
  process.exit(1);
}
  ...
  try {
    ...
    try {
      ...
      if (checkResponse.data.data && checkResponse.data.data.length > 0) {
        console.log(
          `Directus user with email ${userData.email} already exists, skipping creation`
        );
...
async function createInitialInviteCode(adminUserId) {
  ...
  try {
    ...
    if (parseInt(checkResult.rows[0].count) > 0) {
      console.log(
        "Invite codes already exist, skipping creation of initial code"
      );
      return;
    }
    ...
    console.log(
      `Initial invite code created: ${code} (valid for 30 days, 5 uses)`
    );
  } catch (error) {
...
async function setupDatabase() {
  ...
  try {
    ...
    if (userCheckResult.rows.length > 0) {
      console.log(
        `Admin user '${config.adminUsername}' already exists in database.`
      );
      ...
      console.log(
        `Admin user '${config.adminUsername}' created successfully in database.`
      );
    }
    ...
    try {
      ...
      console.log(
        "Admin user setup in both PostgreSQL and Directus completed successfully."
      );
    } catch (directusError) {
      console.error("Error setting up Directus user:", directusError.message);
      console.warn(
        "Continuing with database setup only. You may need to manually create the user in Directus."
      );
    }
